    ...
    <div class="container">
        <div class="row">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Search products..." v-model="name">
            ...
            <form class="shop__filter">
                ...
                <div class="radio">
                    <input type="radio" name="shop-filter__price" id="shop-filter-price_1" value="" checked="" @click="checkPrice = !checkPrice">
                    <label for="shop-filter-price_1">All</label>
                </div>
                <div class="radio">
                    <input type="radio" name="shop-filter__price" id="shop-filter-price_4" value="specify" :checked="checkPrice" @click="checkPrice = !checkPrice">
                    <label for="shop-filter-price_4">Other (specify)</label>
                </div>
                <div class="form-group shop-filter__price" v-if="checkPrice">
                    <div class="row">
                        <div class="col-xs-1">
                            <label for="shop-filter-price_from" class="sr-only"></label>
                            <input id="shop-filter-price_from" type="number" min="0" class="form-control" placeholder="From" v-model="startPrice">
                        </div>
                        <div class="col-xs-1">
                            <label for="shop-filter-price_to" class="sr-only"></label>
                            <input id="shop-filter-price_to" type="number" min="0" class="form-control" placeholder="To" v-model="endPrice">
                        </div>
                        <div class="col-xs-1">
                            <button type="button" class="btn btn-block btn-primary btn-default" @click="searchPrice">Go</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="radio">
                    <input type="radio" name="brand" id="0" @click="brandCheck(0)" checked>
                    <label for="0">All</label>
                </div>
                <div v-for="brand in brands" :key="brand.brand_id">
                    <div class="radio">
                        <input type="radio" name="brand" :id="brand.brand_id" @click="brandCheck(brand.brand_id)">
            ...
            <div class="row mt-5" v-if="isLoading">
                <div class="col-sm-6 col-md-4" v-for="product in productsShow" :key="product.product_id" >
                    <div class="shop__thumb">
                        ...
                        <div class="shop-thumb__img">
                            <img :src="'data:image/png;base64,' + product.product_thumbnail.image_base64" :alt="product.product_name"/>
                        </div>
            ...
            <div class="row">
                ...
                <nav aria-label="Page navigation example" class="pull-right" v-if="pageShow > 1">
                    <ul class="pagination">
                        <li class="page-item">
                        ...
                        <li v-for="p in pageShow" :key="p" :class="page == p ? 'page-item active': 'page-item'"><a class="page-link" @click="pagination(p)">{{p}}</a></li>
...
const BrandRepository = RepositoryFactory.get('brand')
...
export default {
    ...
    methods: {
        fetch() {
            Promise.all([ProductsRepository.get(), BrandRepository.get()]).then(
                response => {
                    this.products = response[0].data.products;
                    this.countProduct()
        ...
        countProduct() {
            this.pageShow = parseInt(this.products.length / 12) + 1
            if(this.products.length > 12 * this.page){
                this.productsShow = this.products.slice((this.page - 1) * 12, 12 * this.page)
            } else{
                this.productsShow = this.products.slice((this.page - 1) * 12, this.products.length)
            }
        },
        brandCheck(id) {
            if(id == 0){
                ...
                ProductsRepository.getProductByBrand(id).then(
                    response => {
        ...
        searchPrice() {
            ProductsRepository.getProductByPrice(this.startPrice, this.endPrice).then(
                response => {
                    this.products = response.data.products
                    this.countProduct()
                }
        ...
        searchName() {
            ProductsRepository.getProductByName(this.name).then(
                response => {
                    this.products = response.data.products
                    this.countProduct()
